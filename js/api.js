    ...
    for (let i = 0; i < totalRecipes; i++) {
        ...
        try {
            ...
            if (recipe && recipe.id) {
                ...
                recipeCache[mealName] = recipe; // Add to local cache copy
                // Immediately save the updated cache to Firestore to prevent data loss on interruption
                await updateState({ recipeCache: recipeCache }, true);
            } else {
